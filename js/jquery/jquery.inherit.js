...
$.inherit = function() {
	...
	var withMixins = $.isArray(arguments[0]),
		...
		result = props.__constructor || basePtp.__constructor?
			function() {
	...
	var propName, i = 0;
	while(propName = propList[i++]) {
		if(hasBase
			&& $.isFunction(basePtp[propName]) && $.isFunction(props[propName])
			&& (!hasIntrospection || props[propName].toString().indexOf('.__base') > -1)) {
			...
			resultPtp[propName] = props[propName];
		}
	};
